...
EndRange <-  read_sf("Shapes/OverwinterBuffer240kilo.shp")
...
output <- foreach(j = 1:num.agents, .combine=rbind,
                  .packages = c('circular', 'CircStats', 'raster', 'BBmisc', 'sf', 'terra')) %dopar% {
                    ...
                    repeat{
                      ...
                      #Normal Vector Day
                      if(length(unlist(is_within[[1]])) == 0 & nd < (stop_migration/2)){
                        #Normal vector day
                        repeat{
                        ...
                        }else if(nd>=(stop_migration/2) & length(unlist(is_within[[1]])) == 0){
                          #South vector day
                          repeat{
